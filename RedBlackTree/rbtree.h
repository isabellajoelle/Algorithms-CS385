...
public:
    ...
    size_t diameter() const {
    	...
    	size_t thruRoot =  diameter(root_->left) + diameter(root_->right) + 1;
    	size_t maxSubtree = std::max(diameter(root_->left), diameter(root_->right));

    	return std::max(thruRoot, maxSubtree);
    }
...
private:
    ...
    int diameter(Node<K, V> *node) const {
    	...
    	int LD = 1 + diameter(node->left);
    	int RD = 1 + diameter(node->right);

    	int path = LH+RH;
    	int maxSubtree = std::max(LD, RD);

    	return std::max(path, maxSubtree);
    ...
    size_t null_count(Node<K, V> *node) const {
    	...
    	if(node == nullptr){
    		...
    		return 1 + null_count(node->right);
    	}else if(node->left != nullptr&& node->right == nullptr){
    		return 1 + null_count(node->left);
    	}else{
